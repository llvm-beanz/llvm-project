...
unsigned BitcodeWriterBase::getEncodedCastOpcode(unsigned Opcode) {
  switch (Opcode) {
...
unsigned BitcodeWriterBase::getEncodedUnaryOpcode(unsigned Opcode) {
  switch (Opcode) {
...
unsigned BitcodeWriterBase::getEncodedBinaryOpcode(unsigned Opcode) {
  switch (Opcode) {
  default: llvm_unreachable("Unknown binary instruction!");
...
unsigned BitcodeWriterBase::getEncodedRMWOperation(AtomicRMWInst::BinOp Op) {
  switch (Op) {
...
void BitcodeWriterBase::writeStringRecord(BitstreamWriter &Stream,
...
uint64_t BitcodeWriterBase::getAttrKindEncoding(Attribute::AttrKind Kind) {
  switch (Kind) {
...
unsigned
BitcodeWriterBase::getEncodedLinkage(const GlobalValue::LinkageTypes Linkage) {
  switch (Linkage) {
...
unsigned BitcodeWriterBase::getEncodedLinkage(const GlobalValue &GV) {
...
unsigned BitcodeWriterBase::getEncodedVisibility(const GlobalValue &GV) {
...
unsigned BitcodeWriterBase::getEncodedDLLStorageClass(const GlobalValue &GV) {
...
unsigned BitcodeWriterBase::getEncodedThreadLocalMode(const GlobalValue &GV) {
...
unsigned BitcodeWriterBase::getEncodedComdatSelectionKind(const Comdat &C) {
...
uint64_t BitcodeWriterBase::getOptimizationFlags(const Value *V) {
  uint64_t Flags = 0;
...
void BitcodeWriterBase::emitSignedInt64(SmallVectorImpl<uint64_t> &Vals,
                                        uint64_t V) {
  if ((int64_t)V >= 0)
    Vals.push_back(V << 1);
  else
...
void BitcodeWriterBase::emitWideAPInt(SmallVectorImpl<uint64_t> &Vals,
                                      const APInt &A) {
...
static void writeFunctionTypeMetadataRecords(BitstreamWriter &Stream,
  ...
  auto WriteRange = [&](ConstantRange Range) {
    ...
    BitcodeWriterBase::emitSignedInt64(Record, *Range.getLower().getRawData());
    BitcodeWriterBase::emitSignedInt64(Record, *Range.getUpper().getRawData());
  };
...
void BitcodeWriterBase::writeIdentificationBlock(BitstreamWriter &Stream) {
