...
Expected<Symbol *> COFFLinkGraphBuilder::createDefinedSymbol(
  ...
  if (Symbol.getStorageClass() == COFF::IMAGE_SYM_CLASS_STATIC) {
    ...
    if (!Definition || !isComdatSection(Section) ||
        ComdatLeaders[Symbol.getSectionNumber()]) {
    ...
    return createCOMDATLeaderSymbol(SymIndex, Symbol, Definition);
  }
...
Expected<Symbol *> COFFLinkGraphBuilder::createCOMDATLeaderSymbol(
  ...
  ComdatLeaders[Symbol.getSectionNumber()] = {SymIndex, L};
...
COFFLinkGraphBuilder::exportCOMDATSymbol(COFFSymbolIndex SymIndex,
  ...
  auto Leader = *ComdatLeaders[Symbol.getSectionNumber()];
  COFFSymbolIndex TargetIndex = Leader.Index;
  Linkage L = Leader.Linkage;
