...
private:
  // The first symbol of COMDAT sequence.
  // This symbol contains the actual definition of the symbol and proper linkage
  // mode specified by the selection type. This symbol will be exported when
  // second symbol of COMDAT sequence is processed which contains the external
  // name of the symbol.
  struct ComdatLeader {
    COFFSymbolIndex Index;
    Linkage Linkage;
  };
  std::vector<Optional<ComdatLeader>> ComdatLeaders;
  ...
  Expected<Symbol *> createCOMDATLeaderSymbol(
