...
define noundef <4 x float> @quad_read_lane_at_v4f32(<4 x float> noundef %expr, i32 noundef %idx) {
entry:
; CHECK: call float @dx.op.quadReadLaneAt.f32(i32 122, float %expr.i0, i32 %idx)
; CHECK: call float @dx.op.quadReadLaneAt.f32(i32 122, float %expr.i1, i32 %idx)
; CHECK: call float @dx.op.quadReadLaneAt.f32(i32 122, float %expr.i2, i32 %idx)
; CHECK: call float @dx.op.quadReadLaneAt.f32(i32 122, float %expr.i3, i32 %idx)
  %ret = call <4 x float> @llvm.dx.quad.read.lane.at.v4f32(<4 x float> %expr, i32 %idx)
  ret <4 x float> %ret
}

declare <2 x half> @llvm.dx.quad.read.lane.at.v2f16(<2 x half>, i32)
declare <3 x i32> @llvm.dx.quad.read.lane.at.v3i32(<3 x i32>, i32)
declare <4 x float> @llvm.dx.quad.read.lane.at.v4f32(<4 x float>, i32)
