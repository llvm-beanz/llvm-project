...
; Unlike WaveReadLaneAt/OpGroupNonUniformShuffle, this backs a broadcast whose
; source lane is required by the SPIR-V spec to be dynamically uniform, so it
; lowers to OpGroupNonUniformBroadcast instead.
...
entry:
; CHECK:   %[[#fret:]] = OpGroupNonUniformBroadcast %[[#f32]] %[[#scope]] %[[#fexpr]] %[[#idx1]]
  %0 = call float @llvm.spv.wave.broadcast.f32(float %fexpr, i32 %idx)
  ret float %0
}
...
entry:
; CHECK:   %[[#iret:]] = OpGroupNonUniformBroadcast %[[#uint]] %[[#scope]] %[[#iexpr]] %[[#idx2]]
  %0 = call i32 @llvm.spv.wave.broadcast.i32(i32 %iexpr, i32 %idx)
  ret i32 %0
}
...
entry:
; CHECK:   %[[#vbret:]] = OpGroupNonUniformBroadcast %[[#v4_bool]] %[[#scope]] %[[#vbexpr]] %[[#idx3]]
  %0 = call <4 x i1> @llvm.spv.wave.broadcast.v4i1(<4 x i1> %vbexpr, i32 %idx)
  ret <4 x i1> %0
}
...
entry:
; CHECK:   %[[#vbret:]] = OpGroupNonUniformBroadcast %[[#v4_float]] %[[#scope]] %[[#vfexpr]] %[[#idx4]]
  %0 = call <4 x float> @llvm.spv.wave.broadcast.v4f32(<4 x float> %vfexpr, i32 %idx)
  ret <4 x float> %0
}

declare float @llvm.spv.wave.broadcast.f32(float, i32)
declare i32 @llvm.spv.wave.broadcast.i32(i32, i32)
declare <4 x i1> @llvm.spv.wave.broadcast.v4i1(<4 x i1>, i32)
declare <4 x float> @llvm.spv.wave.broadcast.v4f32(<4 x float>, i32)
