...
TEST(PhysicalDeviceInfo,
  ...
  // `wideLines`/`largePoints`/`alphaToOne`/`sampleRateShading`/
  // `vertexPipelineStoresAndAtomics`/`fragmentStoresAndAtomics` are the
  ...
  // `vertexPipelineStoresAndAtomics`/`fragmentStoresAndAtomics` flip to
  // `VK_TRUE` as of roadmap H7g: an investigation of the CPU lowering
  // pipeline found no stage-conditioned gate on a storage-buffer store
  // anywhere, confirmed by a real vertex/fragment-stage storage-buffer
  // write draw (`DrawTest.cpp`'s `VertexStageWritesStorageBuffer`/
  // `FragmentStageWritesStorageBuffer`). Roadmap H7h implemented a real
  // `gl_ClipDistance`/`gl_CullDistance` vertex-stage, static-index
  // consumer (`Executor.cpp`'s `clipTriangle`/`RasterizePrimitives`,
  // confirmed by `ExecutorTest.cpp`'s new
  // `ClipsATriangleAgainstAWrittenClipDistance`/
  // `CullsATriangleWhenCullDistanceIsNegativeForEveryVertex`) but left
  // `shaderClipDistance`/`shaderCullDistance` at `VK_FALSE`: a real CTS
  // re-run shows only 16 of this feature's ~330 mandatory cases passing
  // (the vertex-stage, static-index, non-fragment-shader-read subset);
