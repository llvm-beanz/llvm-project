...
; roadmap L48). Roadmap L50d then added a real, nonzero `ConstOffset` for
; `Plain2D`/`Array2D` (see spirv-resource-lowering-image-samplecmp.ll's
; own `samplecmp_offset` case and spirv-resource-lowering-image-samplecmp-
; shapes.ll's own `samplecmp_array2d_offset` case) -- only the `MinLod`
; clamp narrowing below remains: it hits that one narrowing and so is left
; entirely unrewritten, matching spirv-resource-lowering-unsupported.ll's
; own "left untouched" contract.
