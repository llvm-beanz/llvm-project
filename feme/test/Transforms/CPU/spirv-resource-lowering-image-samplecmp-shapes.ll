...
; `samplecmp_clamp`, a `Bias` operand, and `Plain1D`/`Array1D` shadow
; sampling all remain unsupported -- `samplecmp_clamp` against these four
; shapes is now covered instead by roadmap L52(c), see
; spirv-resource-lowering-image-samplecmp-clamp.ll. A nonzero `ConstOffset` (roadmap L50d) is now supported for
...
define float @samplecmp_array2d_offset(<4 x float> %coord, float %dref) {
  ...
  %samp = call target("spirv.Sampler")
      @llvm.spv.resource.handlefrombinding.tsamp.array2d(i32 0, i32 1, i32 1, i32 0, ptr null)
  ; CHECK: call float @feme.cpu.image.samplecmp.2darray.f32(ptr %image_heap, i32 %image_heap_count, ptr %sampler_heap, i32 %sampler_heap_count, i32 {{[0-9]+}}, i32 {{[0-9]+}}, float %{{.*}}, float %{{.*}}, float %{{.*}}, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, i1 false, float %dref, float 0.000000e+00, i32 1, i32 -1, float -inf, i1 true)
  %r = call float @llvm.spv.resource.samplecmp(
      target("spirv.Image", float, 1, 2, 1, 0, 1, 0) %img,
      target("spirv.Sampler") %samp, <4 x float> %coord, float %dref,
