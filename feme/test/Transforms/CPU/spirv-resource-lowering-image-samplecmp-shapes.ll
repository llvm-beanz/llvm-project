...
; `samplecmp_clamp`, a nonzero `ConstOffset`, a `Bias` operand, and
; `Plain1D`/`Array1D` shapes all remain unsupported -- see
...
define float @samplecmp_array2d(<4 x float> %coord, float %dref) {
  ...
  ; CHECK: call float @feme.cpu.image.samplecmp.2darray.f32(ptr %image_heap, i32 %image_heap_count, ptr %sampler_heap, i32 %sampler_heap_count, i32 {{[0-9]+}}, i32 {{[0-9]+}}, float %[[U]], float %[[V]], float %[[LAYER]], float 0.000000e+00, i1 false, float %dref, i1 true)
