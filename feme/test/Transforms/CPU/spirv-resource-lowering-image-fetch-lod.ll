...
; fourth operand) must be a compile-time zero -- the only value
; `ImageFetchLodPattern` emits today (see `isFetchLevelIntrinsic`'s own
; doc); a real, nonzero offset (`texelFetchOffset()`) remains unstarted
; follow-on work.
...
define <4 x float> @fetch_level_array(<3 x i32> %coord, i32 %lod) {
  ...
  ; CHECK: %[[X:.*]] = extractelement <3 x i32> %coord, i64 0
  ; CHECK: %[[Y:.*]] = extractelement <3 x i32> %coord, i64 1
