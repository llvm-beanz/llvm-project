...
//
// (Roadmap L124(f)) This file used to also cover a second, distinct
// decline case here: a column-select access into a `RowMajor` matrix
// reached through a dxc wrapper's own array
// (`RWStructuredBuffer<matCxR>`). That shape is no longer declined --
// `getMatrixColumnAccess` now recognizes it (via
// `getWrapperArrayMatrixColumnAccess`) -- so it moved to
// spirv-to-llvm-matrix-rowmajor-wrapper-array-column.mlir as a positive
// test instead.
