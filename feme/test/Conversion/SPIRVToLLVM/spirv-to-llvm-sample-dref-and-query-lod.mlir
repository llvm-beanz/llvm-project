...
// the non-`Dref` sibling pattern. `Bias` is not supported here (unlike the
// non-`Dref` sibling): LLVM's SPIRV backend has no depth-comparison
// sampling intrinsic combining a bias with a comparison at all.
