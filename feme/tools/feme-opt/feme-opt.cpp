...
void registerFeMePasses(PassBuilder &PB) {
  ...
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
         ArrayRef<PassBuilder::PipelineElement>) {
        if (Name != feme::nvptx::RaisedLoweringPass::name())
          return false;
        MPM.addPass(feme::nvptx::RaisedLoweringPass());
        return true;
      });
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
         ArrayRef<PassBuilder::PipelineElement>) {
        if (Name != feme::nvptx::ResourceLoweringPass::name())
          return false;
        MPM.addPass(feme::nvptx::ResourceLoweringPass());
        return true;
      });
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
  ...
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
         ArrayRef<PassBuilder::PipelineElement>) {
        if (Name != feme::cpu::RootConstantLoweringPass::name())
          return false;
        MPM.addPass(feme::cpu::RootConstantLoweringPass());
        return true;
      });
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
  ...
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
         ArrayRef<PassBuilder::PipelineElement>) {
        if (Name != feme::cpu::SPIRVUnmergeResourceLoadsPass::name())
          return false;
        MPM.addPass(feme::cpu::SPIRVUnmergeResourceLoadsPass());
        return true;
      });
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
  ...
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
         ArrayRef<PassBuilder::PipelineElement>) {
        if (Name != feme::cpu::SPIRVBuiltinFoldingPass::name())
          return false;
        MPM.addPass(feme::cpu::SPIRVBuiltinFoldingPass());
        return true;
      });
  PB.registerPipelineParsingCallback(
      [](StringRef Name, ModulePassManager &MPM,
