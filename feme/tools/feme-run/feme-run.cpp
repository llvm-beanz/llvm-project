...
Expected<ResourceFormat> parseResourceFormat(StringRef Format) {
  ResourceFormat Result =
      StringSwitch<ResourceFormat>(Format)
          ...
          .Case("e5b9g9r9_ufloat", ResourceFormat::E5B9G9R9_UFLOAT)
...
uint32_t imageFormatElementSize(ResourceFormat Format) {
  ...
  case ResourceFormat::A4B4G4R4_UNORM:
    return 2;
  // (Roadmap H7r) The remaining core-1.0 packed 16-bit formats: also 2
  // bytes each, same as `A1B5G5R5_UNORM` above.
  ...
  case ResourceFormat::A1R5G5B5_UNORM:
    return 2;
  // (Roadmap H19j) `R8_{UNORM,SNORM,UINT,SINT}`: one byte each.
  ...
  // (Roadmap H8j) ETC2/EAC block-compressed formats: same "no per-texel
  // size" status as ASTC/BC above.
