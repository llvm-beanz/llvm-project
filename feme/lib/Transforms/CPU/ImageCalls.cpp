...
Function *feme::cpu::getOrInsertImageCall(Module &M, ImageCallKind Kind) {
  ...
  case ImageCallKind::Sample2D:
    ...
    break;
  case ImageCallKind::SampleCmp2D:
    // Same as Sample2D's own (image_heap..v) prefix, plus a trailing
    // `dref`, `bias` (roadmap L52(b); SPIR-V's own `Bias` image operand,
    // mirroring Sample2D's own operand of the same name),
    // `offset_x`/`offset_y` (roadmap L50d; SPIR-V's own `ConstOffset`
    // image operand, mirroring Sample2D's own operand of the same name),
    // `min_lod_clamp` (roadmap L52(c); SPIR-V's own `MinLod` image
    // operand, mirroring Sample2D's own operand of the same name), and
    // `mask`; returns `float`.
    // (image_heap, image_heap_count, sampler_heap, sampler_heap_count,
    //  image_index, sampler_index, u, v, lod, use_explicit_lod, dref,
    //  bias, offset_x, offset_y, min_lod_clamp, mask) -> float
    FTy = FunctionType::get(F32Ty,
                            {PtrTy, I32Ty, PtrTy, I32Ty, I32Ty, I32Ty, F32Ty,
                             F32Ty, F32Ty, I1Ty, F32Ty, F32Ty, I32Ty, I32Ty,
                             F32Ty, I1Ty},
                            /*isVarArg=*/false);
    break;
  case ImageCallKind::Load2D:
  ...
  case ImageCallKind::SampleCmpCubeArray:
    ...
    // L48).
    // (image_heap, image_heap_count, sampler_heap, sampler_heap_count,
    //  image_index, sampler_index, dir_x, dir_y, dir_z, array_layer, lod,
    //  use_explicit_lod, dref, bias, min_lod_clamp, mask) -> float
    FTy = FunctionType::get(F32Ty,
                            {PtrTy, I32Ty, PtrTy, I32Ty, I32Ty, I32Ty, F32Ty,
    ...
    break;
  case ImageCallKind::Sample1D:
    // Roadmap L52a: a single float u coordinate, no offset (mirroring
    // Sample2DArray's own simpler scope). Roadmap L61(c) adds a real
    // Bias/min_lod_clamp pair, mirroring Sample2D's own operand order
    // (lod, use_explicit_lod, bias, ..., min_lod_clamp, mask). Roadmap
    // L63 adds a real du_dx/du_dy screen-space derivative pair, mirroring
    // Sample2D's own du_dx/du_dy/dv_dx/dv_dy placement immediately after
    // the coordinate.
    // (image_heap, image_heap_count, sampler_heap, sampler_heap_count,
    //  image_index, sampler_index, u, du_dx, du_dy, lod, use_explicit_lod,
    //  bias, min_lod_clamp, mask) -> <4 x float>
    FTy = FunctionType::get(
        V4F32Ty,
        {PtrTy, I32Ty, PtrTy, I32Ty, I32Ty, I32Ty, F32Ty, F32Ty, F32Ty, F32Ty,
         I1Ty, F32Ty, F32Ty, I1Ty},
  ...
  case ImageCallKind::SampleCmp1D:
    ...
    // (image_heap, image_heap_count, sampler_heap, sampler_heap_count,
    //  image_index, sampler_index, u, lod, use_explicit_lod, dref, bias,
    //  min_lod_clamp, mask) -> float
    FTy = FunctionType::get(F32Ty,
                            {PtrTy, I32Ty, PtrTy, I32Ty, I32Ty, I32Ty, F32Ty,
                             F32Ty, I1Ty, F32Ty, F32Ty, F32Ty, I1Ty},
                            /*isVarArg=*/false);
    break;
  case ImageCallKind::SampleCmpArray1D:
...
CallInst *feme::cpu::createSampleCmp2D(
    ...
    Value *SamplerIndex, Value *U, Value *V, Value *Lod, Value *UseExplicitLod,
  ...
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             ...
                             V, Lod, UseExplicitLod, Dref, Bias, OffsetX,
...
CallInst *feme::cpu::createSampleCmpArray2D(
    ...
    Value *UseExplicitLod, Value *Dref, Value *Bias, Value *OffsetX,
    Value *OffsetY, Value *MinLodClamp, Value *Mask, const Twine &Name) {
  Module *M = Builder.GetInsertBlock()->getModule();
  Function *F = getOrInsertImageCall(*M, ImageCallKind::SampleCmpArray2D);
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             Env.SamplerHeapCount, ImageIndex, SamplerIndex, U,
                             V, ArrayLayer, Lod, UseExplicitLod, Dref, Bias,
...
CallInst *feme::cpu::createSampleCmpCube(
    IRBuilderBase &Builder, const ImageCallEnv &Env, Value *ImageIndex,
    Value *SamplerIndex, Value *DirX, Value *DirY, Value *DirZ, Value *Lod,
  ...
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             Env.SamplerHeapCount, ImageIndex, SamplerIndex,
                             DirX, DirY, DirZ, Lod, UseExplicitLod, Dref, Bias,
                             MinLodClamp, Mask},
                            Name);
}
...
CallInst *feme::cpu::createSampleCmpCubeArray(
  ...
  return Builder.CreateCall(
      ...
      {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
       Env.SamplerHeapCount, ImageIndex, SamplerIndex, DirX, DirY, DirZ,
       ArrayLayer, Lod, UseExplicitLod, Dref, Bias, MinLodClamp, Mask},
      Name);
}
...
CallInst *feme::cpu::createSample1D(IRBuilderBase &Builder,
                                    ...
                                    Value *DUdX, Value *DUdY, Value *Lod,
                                    Value *UseExplicitLod, Value *Bias,
                                    Value *MinLodClamp, Value *Mask,
                                    const Twine &Name) {
  Module *M = Builder.GetInsertBlock()->getModule();
  ...
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             Env.SamplerHeapCount, ImageIndex, SamplerIndex, U,
                             DUdX, DUdY, Lod, UseExplicitLod, Bias,
                             MinLodClamp, Mask},
                            Name);
}

CallInst *feme::cpu::createSample1DArray(IRBuilderBase &Builder,
                                         const ImageCallEnv &Env,
                                         Value *ImageIndex,
                                         Value *SamplerIndex, Value *U,
                                         Value *ArrayLayer, Value *DUdX,
...
CallInst *feme::cpu::createSampleCmp1D(IRBuilderBase &Builder,
  ...
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             Env.SamplerHeapCount, ImageIndex, SamplerIndex, U,
                             Lod, UseExplicitLod, Dref, Bias, MinLodClamp,
                             Mask},
                            Name);
}
...
CallInst *feme::cpu::createSampleCmpArray1D(
  ...
  return Builder.CreateCall(F,
                            {Env.ImageHeap, Env.ImageHeapCount, Env.SamplerHeap,
                             Env.SamplerHeapCount, ImageIndex, SamplerIndex, U,
                             ArrayLayer, Lod, UseExplicitLod, Dref, Bias,
                             MinLodClamp, Mask},
                            Name);
}
...
std::optional<MatchedImageCall> feme::cpu::matchImageCall(const CallInst &CI) {
  ...
  case ImageCallKind::SampleCmp2D:
    ...
    Result.OffsetY = CI.getArgOperand(13);
  ...
  case ImageCallKind::SampleCmpArray2D:
    ...
    Result.OffsetY = CI.getArgOperand(14);
  ...
  case ImageCallKind::Sample1D:
    ...
    Result.UseExplicitLod = CI.getArgOperand(10);
    Result.Bias = CI.getArgOperand(11);
    Result.MinLodClamp = CI.getArgOperand(12);
  ...
  case ImageCallKind::SampleCmp1D:
    ...
    Result.Bias = CI.getArgOperand(10);
