...
/// Whether \p Format is one of the unsigned- or signed-integer color-
/// attachment formats roadmap H8p (the original 7) and H8s (12 more,
/// found by a real CTS re-run against feme's own ICD) add real
/// fragment-output write support for (`R8G8B8A8_{UINT,SINT}`,
/// `R10G10B10A2_UINT`, `R16_{UINT,SINT}`, `R16G16_{UINT,SINT}`,
/// `R8_{UINT,SINT}`, `R8G8_{UINT,SINT}`, `R16G16B16A16_{UINT,SINT}`,
/// `R32G32B32A32_{UINT,SINT}`, `R32_{UINT,SINT}`, `R32G32_{UINT,SINT}`), a
/// real `deqp-vk`-confirmed `COLOR_ATTACHMENT_BIT` gap H8e found but did
/// not fix (`Executor.cpp`'s `executeDraws` used to hard-reject any
/// non-`Float` fragment output outright, regardless of the target
/// attachment's own format). Not a contiguous enum range (see
...
/// (Format.cpp). None of these formats supports
...
constexpr bool isIntegerColorAttachmentFormat(ResourceFormat Format) {
  ...
  // CTS re-run against feme's own ICD found still missing
  // `COLOR_ATTACHMENT_BIT`: each already has a real `packClearColor`/
  // `unpackColor` case (`ImageFixture.cpp`, used already by the
  // vertex-fetch/texel-buffer/storage-image paths), and
  // `readFragmentColorInt`/`expectedColorComponentType` (`Executor.cpp`)
  // are already generic over any integer-typed output's width/signedness,
  // so this predicate (plus `isUnsignedIntegerColorAttachmentFormat`
  // below and `RenderPass.cpp`'s `isSupportedColorAttachmentFormat`) is
  // a complete fix with no other code changes needed.
...
/// The unsigned-integer half of `isIntegerColorAttachmentFormat`'s own
/// formats above -- `false` for every signed-integer half and every
