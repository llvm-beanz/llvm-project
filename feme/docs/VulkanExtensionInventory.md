...
| `VK_EXT_mesh_shader` | Advertised | roadmap H29r: a mesh entry's `gl_PrimitiveTriangleIndicesEXT`/`gl_PrimitiveLineIndicesEXT`/`gl_PrimitivePointIndicesEXT` output is now classified as `SignatureSystemValue::PrimitiveIndices` and lowered into `FemeMeshArgs::PrimitiveIndices`; before this every emitted primitive degenerated to `(vertex 0, vertex 0, vertex 0)` and never rasterized |
...
| `VK_KHR_compute_shader_derivatives` | Implemented (`DerivativeGroupLinearKHR` only) | roadmap L69: now advertised, with `computeDerivativeGroupLinear=VK_TRUE`/`computeDerivativeGroupQuads=VK_FALSE` feature bits and both execution modes recognized at pipeline-creation time; `DerivativeGroupLinearKHR` compiles and validates end to end (feme's flat/linear compute-stage lane assignment already matches this mode's own spec-defined grouping). `DerivativeGroupQuadsKHR` is deliberately rejected with a named error (roadmap L69(a): needs a real 2x2-spatial-tile lane-assignment redesign this target does not have). Real CTS payoff is currently masked by an unrelated, pre-existing gap (roadmap L70: all compute-stage image sampling fails resource-handle normalization regardless of derivative use) |
