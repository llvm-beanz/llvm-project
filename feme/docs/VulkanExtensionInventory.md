...
| `VK_EXT_primitives_generated_query` | Advertised (including non-zero streams) | roadmap H21d: real primitive-count query semantics reusing the existing pipeline-statistics `QueryPool` infrastructure; `primitivesGeneratedQuery` feature `VK_TRUE`, `primitivesGeneratedQueryWithRasterizerDiscard` stays `VK_FALSE` (blocked on roadmap H21g), `WithNonZeroStreams` is now `VK_TRUE` (roadmap H173(b) -- `Executor.cpp`'s own `GeometryShaderPrimitives`/`ClippingInvocations` accounting already counts every emitted primitive regardless of which stream rasterization selects) |
...
| `VK_EXT_transform_feedback` | Advertised (independent simultaneous multi-stream capture, including from a geometry stage; `primitives_generated_query` implemented, including non-zero streams) | roadmap H21c: real vertex-shader-output capture to bound XFB buffers; H21e: geometry-stage output capture, `RasterizationStream` selection; H173(b): every geometry-stage stream carrying an `XfbBuffer`-tagged element now captures independently and simultaneously (not just the `RasterizationStream`-selected one); `transformFeedback`/`transformFeedbackDraw`/`transformFeedbackRasterizationStreamSelect`/`geometryStreams`/`primitivesGeneratedQuery`/`primitivesGeneratedQueryWithNonZeroStreams` all `VK_TRUE`, `maxTransformFeedbackStreams` raised from the spec-floor `1` to `16`; real CTS confirms `dEQP-VK.transform_feedback.simple.multistreams_{1,3,6,14}` (4/4 Pass, from `NotSupported`). Newly exposed by this same flip (previously `NotSupported`, unrelated to the capture fix itself): `streams_N`/`streams_clipdistance_N`/`streams_culldistance_N` point-input-geometry image mismatches (roadmap H173(c)) and `multistreams_same_location_N`'s unhandled SPIR-V `Component` decoration (roadmap H173(d)) |
