...
- FeMe source revision under test: `d627b4d3e286`
...
- Build: `Release`, `LLVM_ENABLE_ASSERTIONS=ON`, C and C++ compilation through
  `ccache`
- `check-feme`: 3,300 passed, 3 unsupported, 0 failed
- Registry used by the inventories: `VK_HEADER_VERSION` 358

The inventory audit performed with this run reports 87 of 150 Vulkan core
feature bits advertised, 49 of 86 promoted extensions implemented, and 41
extensions advertised by name. See
[Vulkan14FeatureInventory.md](Vulkan14FeatureInventory.md) and
[VulkanExtensionInventory.md](VulkanExtensionInventory.md).

## Method

The CTS generated 3,244,369 cases in 54 top-level `dEQP-VK` groups. Every case
was attempted against the explicit build-tree ICD:
...
Every CTS invocation ran from the Vulkan module directory with
`--deqp-shadercache=disable`. The initial pass used 1,800-case process batches,
then incomplete tails were recovered in 200-case, 20-case, and finally
single-case processes. `feme/utils/vk_cts_reconcile.py` accepted a result only
when its QPA record had a matching `#endTestCaseResult`; contradictory retries
were rejected. The isolated final pass classified every remaining incomplete
case, leaving no case unattempted.

`deqp-vk` returns nonzero for ordinary failures, so outcomes were derived from
QPA records and process status rather than the executable's exit code. A case
is **Crashed** when its isolated process ended after starting the case but
before recording a result, and **Timed out** when its isolated process exceeded
the final 60-second limit. This accounting produced 3,244,233 complete QPA
records plus 120 crashes and 16 timeouts.

## G2(a) baseline comparison

The checked-in expected-failure baseline contains 160,248 ordinary `Fail`
cases from FeMe revision `aa5742ca7ed1`. Of those, 68,389 still fail and
91,859 are resolved. The current run has 13,875 failures absent from that
baseline, for a net reduction from 160,461 failures in the preceding full-run
report to 82,264. Crashes, timeouts, and incomplete records remain excluded
from the expected-failure baseline.

The largest improvement is `pipeline`: 83,698 additional passes and 79,178
fewer failures. `draw`, `dynamic_state`, `compute`, `multiview`, and `ssbo` now
have no ordinary failures. The largest newly unexpected clusters are
`binding_model.shader_access` (11,832), `ubo` (707), `glsl.texture_gather`
(474), and `mesh_shader.ext` (85); roadmap L141-L144 track them explicitly.
...
| Pass | 591,111 | 18.2196% |
| Fail | 82,264 | 2.5356% |
| NotSupported | 2,570,802 | 79.2389% |
...
All 54 groups were entered and every case was attempted. Forty-four groups
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| `api` | 267,504 | 267,504 | 99,354 | 15,569 | 152,579 | 0 | 0 | 2 | 0 | 0 |
| `binding_model` | 150,289 | 150,289 | 61,155 | 26,517 | 62,617 | 0 | 0 | 0 | 0 | 0 |
| `clipping` | 308 | 308 | 298 | 0 | 10 | 0 | 0 | 0 | 0 | 0 |
| `compute` | 61,460 | 61,460 | 685 | 0 | 60,775 | 0 | 0 | 0 | 0 | 0 |
...
| `draw` | 29,451 | 29,451 | 3,258 | 0 | 26,193 | 0 | 0 | 0 | 0 | 0 |
...
| `glsl` | 28,420 | 28,420 | 15,554 | 3,139 | 9,699 | 0 | 0 | 28 | 0 | 0 |
| `graphicsfuzz` | 757 | 757 | 612 | 118 | 8 | 0 | 0 | 18 | 1 | 0 |
| `image` | 143,086 | 143,086 | 14,008 | 6,937 | 122,141 | 0 | 0 | 0 | 0 | 0 |
...
| `memory_model` | 18,530 | 18,530 | 26 | 117 | 18,387 | 0 | 0 | 0 | 0 | 0 |
| `mesh_shader` | 28,044 | 28,044 | 366 | 91 | 27,587 | 0 | 0 | 0 | 0 | 0 |
| `multiview` | 838 | 838 | 643 | 0 | 195 | 0 | 0 | 0 | 0 | 0 |
| `pipeline` | 1,172,229 | 1,172,229 | 286,668 | 3,728 | 881,779 | 43 | 9 | 2 | 0 | 0 |
...
| `rasterization` | 15,019 | 15,019 | 378 | 106 | 14,535 | 0 | 0 | 0 | 0 | 0 |
...
| `renderpasses` | 81,188 | 81,188 | 20,692 | 14,127 | 46,369 | 0 | 0 | 0 | 0 | 0 |
...
| `spirv_assembly` | 68,734 | 68,734 | 6,007 | 1,183 | 61,540 | 0 | 0 | 4 | 0 | 0 |
| `ssbo` | 12,225 | 12,225 | 3,242 | 0 | 8,983 | 0 | 0 | 0 | 0 | 0 |
| `subgroups` | 48,705 | 48,705 | 569 | 112 | 48,009 | 0 | 0 | 0 | 15 | 0 |
| `synchronization` | 64,872 | 64,872 | 15,291 | 3,116 | 46,452 | 1 | 0 | 12 | 0 | 0 |
| `synchronization2` | 81,617 | 81,617 | 21,224 | 3,387 | 56,986 | 0 | 0 | 20 | 0 | 0 |
| `tensor` | 2,811 | 2,811 | 0 | 0 | 2,811 | 0 | 0 | 0 | 0 | 0 |
| `tessellation` | 1,114 | 1,114 | 216 | 356 | 518 | 0 | 0 | 24 | 0 | 0 |
| `texture` | 25,669 | 25,669 | 8,771 | 453 | 16,435 | 0 | 0 | 10 | 0 | 0 |
| `transform_feedback` | 133,719 | 133,719 | 4,694 | 2,038 | 126,987 | 0 | 0 | 0 | 0 | 0 |
| `ubo` | 13,240 | 13,240 | 4,979 | 708 | 7,553 | 0 | 0 | 0 | 0 | 0 |
...
Nine groups contain 120 process crashes: `api` (2), `glsl` (28),
`graphicsfuzz` (18), `pipeline` (2), `spirv_assembly` (4), `synchronization`
(12), `synchronization2` (20), `tessellation` (24), and `texture` (10).
The tessellation crashes are the 24 HLSL winding cases. The synchronization
crashes are timeline-semaphore cases, the texture crashes are texel-buffer
formats, and both pipeline crashes are compatible-render-pass cases.

Fifteen of the 16 timeouts are
`subgroups.ballot_broadcast.compute.subgroupbroadcast_*` cases at required
subgroup sizes 64 or 128. The remaining timeout is
`dEQP-VK.graphicsfuzz.cov-nested-structs-function-set-inner-struct-field-return`.
Roadmap L92/L126(a) own the subgroup long poles, L116 owns the graphicsfuzz
outcomes, and L94 owns crash elimination. There is no longer a pipeline or
texture timeout and no recovery tail.
