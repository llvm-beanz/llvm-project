...
**Update (roadmap E19, closed):** a third importer gap, unrelated to control
flow, surfaced while implementing `VK_KHR_shader_non_semantic_info`: MLIR's
SPIR-V deserializer has no generic tolerance for an unrecognized extended-
instruction-set name at all (`OpExtInst` dispatch fails outright for any set
name outside its autogenerated table), even though the SPIR-V specification
itself defines every `NonSemantic.*` set as having no effect on a module's
semantics and explicitly permits a consumer to ignore it. `SPIRVImporter.cpp`
now strips a `NonSemantic.*` `OpExtInstImport` and every `OpExtInst`
referencing it out of the raw word stream before MLIR ever sees it, honoring
that "ignorable" contract at the one layer that can -- see Roadmap.md's E19
row for the full before/after.
...
**Status (roadmap E19):** `VK_EXT_pipeline_creation_feedback`'s
`VkPipelineCreationFeedbackCreateInfo`, chained onto either pipeline-
creation command's `pNext`, is filled by a new shared
`fillPipelineCreationFeedback` (Pipeline.cpp): `VALID_BIT` is always set
(a pipeline that reaches the call already compiled successfully --
compile failures and a cache-miss-with-`FAIL_ON_PIPELINE_COMPILE_
REQUIRED_BIT` both skip it entirely), `APPLICATION_PIPELINE_CACHE_HIT_
BIT` is added exactly when `lookup`/`lookupGraphics` above found a real
hit, and every `duration` (the one overall feedback and each per-stage
one) is honestly `0` -- this ICD has no per-stage compile-timing
instrumentation to report a real number from, and the specification
permits `0` as a valid (if uninformative) answer rather than requiring a
fabricated estimate.
...
their identity-mapping default on every `vkCmdBeginRendering`, per each
command's own man page. `vkCmdSetRenderingAttachmentLocations`' remap is
genuinely honored: the mapping flows from `GraphicsState` (CommandBuffer.h)
through `PreparedDraw::ColorAttachmentLocations` into the executor, whose
fragment-output-to-attachment linkage (Executor.cpp) now searches by each
...
now genuinely consulted too (roadmap F8a): a fragment shader's `subpassInput`
read (`OpTypeImage(Dim=SubpassData)`/`OpImageRead`'s subpass-local form)
converts directly into `feme.stage.subpass.load` calls
(`feme::spirv::SubpassLoadPattern`, SPIRVToLLVMPatterns.cpp) naming the
variable's own `InputAttachmentIndex` decoration, resolved by
`feme::vulkan::runDraw`'s `buildSubpassInputHeap` (CommandBuffer.cpp) into a
`FemeImageDescriptor` per logical index straight from the currently-bound
color/depth/stencil attachments -- not a descriptor-set image -- and read
back by FragmentWrapper.cpp's `lowerFragmentSubpassLoad` at the invocation's
own `FragCoord`, closing the pre-existing gap this document's own
"shader-side SPIR-V `subpassInput` read is still a separate lowering step"
note (above) had flagged for roadmap C5.
...
synthesized border color (`SamplerAddressMode::ClampToBorder` resolving
out of range) via `femeRTApplyImageSwizzle` -- matching core Vulkan's own
rule that a `VK_BORDER_COLOR_*_TRANSPARENT_BLACK`/`*_OPAQUE_WHITE` border
is swizzled by the image view's own component mapping with no
...
attachment still goes through memory rather than a tile-local path -- a
shader-side `subpassInput` read is now lowered (roadmap F8a closed the gap
F8's own dynamic-rendering-local-read *commands* had built atop without
closing -- see that section's own status note above), but always reads
back through ordinary attachment memory rather than genuine tile-local
storage, which this software rasterizer has no equivalent of regardless;
