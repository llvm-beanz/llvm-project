...
| `alloca T` | Implemented (roadmap L84), but scoped narrowly: only for an `alloca` that `feme::cpu::LinearizePass`'s masking (`feme.cpu.masked.load.*`/`.store.*`) has already flagged as holding a per-lane-divergent *value* despite a compile-time-uniform *address* (the case a uniformity analysis can never detect on its own, since it tracks an access's address operand, not the history of which divergently-executed store last wrote through it) -- see `FunctionWidener::widenMaskedAlloca`/`widenMaskedAllocaGEP` in `SIMDize.cpp`. Widens to `alloca [W x T]`, indexed by lane, with a real `<W x ptr>` of distinct per-lane slot addresses; a plain uniformly-accessed `alloca` outside this set is left completely untouched (correct as-is, no widening needed). Roadmap H111(b): a *plain, unconditional* store into such an `alloca` (e.g. a local constant lookup table's own single whole-array initializer, never itself a masked call since it always executes) also needs `widenMaskedAllocaStore` to replicate it once per lane into that lane's own real address -- left as "uniform: unchanged" instead, only one lane's copy ever got initialized, corrupting every other lane's later masked read of it. SROA-back-into-vectors for the uniform case remains unimplemented. |
