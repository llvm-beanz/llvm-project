...
| feature | VK_VERSION_1_0 | `samplerAnisotropy` | no | roadmap H7i implemented a real implicit-LOD/anisotropic-filter kernel (`femeRTPlanImplicitLod` in `FeMeRuntimeCPU.c`: a per-sample mip level from genuine screen-space partial derivatives synthesized at the resource-lowering boundary for a Fragment-stage implicit `Plain2D` sample, `getOrSynthesizeSample2DDerivatives`, then a `MaxAnisotropy`-bounded multi-tap average along the footprint's major axis; scoped to the plain `sampler2D` shape only), but a real CTS re-run found the kernel is never reached: 0/128 `dEQP-VK.texture.filtering_anisotropy.*` (and 0/1698 of the broader, unrelated `dEQP-VK.texture.filtering.2d.*`) fail `vkCreateGraphicsPipelines` on an unrelated, pre-existing `SPIRVResourceLoweringPass` gap (does not recognize a single combined `OpTypeSampledImage`-style `handlefrombinding` call, the shape glslang emits for `uniform sampler2D`), tracked as roadmap H13d. H13d is now closed (pipeline creation succeeds for this shape), but `dEQP-VK.texture.filtering_anisotropy.*` is gated on this feature bit itself, not pipeline creation, so it stays 128/128 `NotSupported` regardless; the broader `filtering.2d.*` re-run also exposed a new, distinct sampling-correctness gap (roadmap H14, real pixel-verification failures once a combined-sampler pipeline actually runs) that would need closing too before the anisotropic kernel's own real behavior could be measured end to end. Stays `VK_FALSE`/`maxSamplerAnisotropy` stays `1.0` until H14 unblocks a real pass |
