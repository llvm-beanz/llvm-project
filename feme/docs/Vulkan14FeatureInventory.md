...
| feature | VK_VERSION_1_0 | `shaderClipDistance` | no | roadmap H7h: real vertex-stage, static-index consumer implemented and CTS-confirmed (16/16 `dEQP-VK.clipping.user_defined.clip_distance.vert.*` non-`_fragmentshader_read` cases pass), but stays `VK_FALSE` since dynamic indexing (H7w), fragment-shader read-back (H7x), and tessellation/geometry-stage writes (H7y) all remain real, separate gaps -- leaving only ~16 of ~330 mandatory CTS cases passing |
| feature | VK_VERSION_1_0 | `shaderCullDistance` | no | roadmap H7h, same reproduction and rationale as `shaderClipDistance` above (`dEQP-VK.clipping.user_defined.clip_cull_distance.vert.*`, same 16/16 non-`_fragmentshader_read` pass / dynamic-index+fragmentshader_read+tess/geom fail split) |
