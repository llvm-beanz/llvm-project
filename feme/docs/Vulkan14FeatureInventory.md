...
| feature | VK_VERSION_1_0 | `shaderStorageImageMultisample` | no | roadmap H7j, tracked to H19: H19a's own scope is explicitly non-multisampled; H19g closed the plain (non-arrayed) 2D storage-image addressing side (`Sample` operand support end-to-end through the compiler and runtime); H19k then closed the `feme-cpu-linearize` control-flow gap H19g's own CTS re-run surfaced (a loop's uniform trip-count check split by `StructurizeCFGPass` into a redundant "Flow" re-derivation block), but a further real CTS re-run (bit forced on to probe) found all 27 previously-blocked cases now hit a distinct, later `feme-cpu-simdize` gap instead ("divergent vector value ... component decomposition is not yet supported for this use") -- tracked as H19l, plus a future arrayed-2D multisample follow-on once H19l lands |
