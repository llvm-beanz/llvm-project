...
| feature | VK_VERSION_1_0 | `textureCompressionBC` | no | roadmap H8i/H8l: complete, directly-unit-tested `BCDecode.h`/`.cpp` (BC1-5, 12 of 16 `VK_FORMAT_BC*` formats) and `BC7Decode.h`/`.cpp` (BC7, 1 more format) decoders now exist -- 13 of 16 BC formats decoded, only BC6H (roadmap H8m) remains -- but neither has any call site yet: `Format.cpp`/`mapVkFormat` has no BC entries, so this bit honestly stays `no` until a future wiring row (a BC counterpart to H8j) flips it |
