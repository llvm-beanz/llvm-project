...
| feature | VK_VERSION_1_0 | `textureCompressionETC2` | no | roadmap H8c: a complete, directly-unit-tested `ETC2Decode.h`/`.cpp` block decoder now exists (all five ETC2 color modes, RGB8A1 punchthrough-alpha, EAC unsigned/signed), but it has zero call sites yet -- `Format.cpp`/`mapVkFormat` has no ETC2/EAC entries, so this bit honestly stays `no` until roadmap H8j wires it into a real consumer |
