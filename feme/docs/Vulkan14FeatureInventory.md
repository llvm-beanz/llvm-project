...
| feature | VK_VERSION_1_0 | `shaderClipDistance` | no |  |
| feature | VK_VERSION_1_0 | `shaderCullDistance` | no |  |
...
| feature | VK_VERSION_1_4 | `dynamicRenderingLocalRead` | yes | roadmap F8/F8a/F8b: `vkCmdSetRenderingAttachmentLocations`/`vkCmdSetRenderingInputAttachmentIndices` are implemented and validated (`CommandBuffer.{h,cpp}`); the attachment-location remap is honored by the executor (`PreparedDraw::ColorAttachmentLocations`, `Executor.cpp`), and a fragment shader's `subpassInput` local read now produces real pixels for a color, depth (`D16_UNORM`/`D32_FLOAT`), or stencil (`S8_UINT`) attachment (`feme::spirv::SubpassLoadPattern` in `SPIRVToLLVMPatterns.cpp`, `FragmentWrapper.cpp`'s `lowerFragmentSubpassLoad`, `CommandBuffer.cpp`'s `buildSubpassInputHeap`, and the CPU runtime's now-decoded depth/stencil texel-unpack cases in `FeMeRuntimeCPU.c`), proven end to end by `DrawTest.SubpassLoadReadsBackThe{Color,Depth,Stencil}AttachmentItWrote` -- only a multisampled attachment remains unread (roadmap F8c); see the two dedicated limit rows below |
...
| limit | VK_VERSION_1_4 | `dynamicRenderingLocalReadMultisampledAttachments` | n/a | roadmap F8c: `buildSubpassInputHeap` now populates a multisampled attachment's heap slot with a correct per-sample layout, but no caller threads an explicit sample index through a subpass load yet (`feme::StageOpKind::SubpassLoad` has no `Sample` operand, and `SubpassLoadPattern` (`SPIRVToLLVMPatterns.cpp`) rejects a `spirv.ImageRead` that carries one) |
