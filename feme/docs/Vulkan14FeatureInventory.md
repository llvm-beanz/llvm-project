...
| feature | VK_VERSION_1_0 | `textureCompressionBC` | no | roadmap H8i/H8l/H8m: complete, directly-unit-tested `BCDecode.h`/`.cpp` (BC1-5, 12 of 16 `VK_FORMAT_BC*` formats), `BC7Decode.h`/`.cpp` (BC7, 1 more format), and `BC6HDecode.h`/`.cpp` (BC6H, the last 2 formats) decoders now exist -- **all 16 of 16 BC formats decoded**, the entire BC1-7 decoder family is complete -- but none has any call site yet: `Format.cpp`/`mapVkFormat` has no BC entries, so this bit honestly stays `no` until a future wiring row (roadmap H8n, a BC counterpart to H8j) flips it |
