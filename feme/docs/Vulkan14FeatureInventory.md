...
| feature | VK_VERSION_1_0 | `shaderStorageImageMultisample` | no | roadmap H7j, tracked to H19: H19a's own scope is explicitly non-multisampled; H19g closed the plain (non-arrayed) 2D storage-image addressing side (`Sample` operand support end-to-end through the compiler and runtime), but a real CTS re-run (bit forced on to probe) found a distinct, larger, unrelated `feme-cpu-linearize` control-flow gap (every real test shader's own `for`-loop-over-samples hits "loop ... has an internal branch in 'Flow'; unsupported") blocking 100% of real closure regardless -- tracked as H19k, plus a future arrayed-2D multisample follow-on once H19k lands |
