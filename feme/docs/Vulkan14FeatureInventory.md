...
| feature | VK_VERSION_1_0 | `samplerAnisotropy` | yes | roadmap H7i: `femeRTPlanImplicitLod` (`FeMeRuntimeCPU.c`) computes a real per-sample mip level from genuine screen-space partial derivatives (synthesized at the resource-lowering boundary for a Fragment-stage implicit `Plain2D` sample, `getOrSynthesizeSample2DDerivatives`), then averages a `MaxAnisotropy`-bounded tap count spread along the footprint's major axis; scoped to the plain `sampler2D` shape only (`Sample2DArray`/`SampleCube`/`SampleCubeArray` implicit samples remain mip level 0). `maxSamplerAnisotropy` raised from `1.0` to `16.0` to match |
