...
| feature | VK_VERSION_1_0 | `shaderStorageImageExtendedFormats` | no | roadmap H7j, tracked to H19: H19a closed the real prerequisite (no `OpImageRead`/`OpImageWrite` lowering existed for a storage image at all) for the plain 2D, non-arrayed, non-multisampled, mandatory-format-floor case only; H19f added real pack/unpack round-trip support (`FeMeRuntimeCPU.c`) for `R16G16B16A16_{SFLOAT,UINT,SINT}`, a first slice of the full mandatory extended-format list, but the bit itself stays `VK_FALSE` until the rest of that list lands -- tracked as H19h |
