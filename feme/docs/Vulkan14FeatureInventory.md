...
| feature | VK_VERSION_1_0 | `shaderClipDistance` | no | roadmap H7h: real vertex-stage, static-index consumer implemented and CTS-confirmed. A real re-measurement (roadmap H53/H54/H55/H56, see "Roadmap H53/H54/H55/H56: measured impact" in VulkanCTSReport.md) now finds the entire non-`_dynamic_index` `vert.*`/`vert_geom.*` bucket (64/64, including `_fragmentshader_read` variants) passes outright -- H54's stage-linkage mismatch and H55's fragment-read-back error are both fixed. The `vert_tess.*`/`vert_tess_geom.*` bucket (64 cases) still fails every case, but H56's own originally-diagnosed compile-time crash is fixed too; the remaining failure is a new, distinct, silent rendering-correctness bug (roadmap H112, open). `_dynamic_index` variants combined with `vert_geom`/`vert_tess_geom` fail separately with an unrelated JIT-symbol-resolution error, not yet triaged. Stays `VK_FALSE` until H112 closes |
| feature | VK_VERSION_1_0 | `shaderCullDistance` | no | roadmap H7h, same reproduction and rationale as `shaderClipDistance` above (`dEQP-VK.clipping.user_defined.clip_cull_distance.*`, same current 64/64 `vert`/`vert_geom` pass, 0/64 `vert_tess`/`vert_tess_geom` fail on roadmap H112, unrelated `_dynamic_index` JIT-symbol failure) |
