...
| feature | VK_VERSION_1_0 | `textureCompressionBC` | no | roadmap H8n: `vkCreateImage` now accepts every `VK_FORMAT_BC*` format, `CommandBuffer.cpp`'s `materializeImageDescriptor` decodes one for real shader sampling, and `ImageOps.cpp`'s `runBlitImage` decodes the RGBA8-shaped subset (BC1/BC2/BC3/BC7) as a blit source -- a real `deqp-vk` run confirms `dEQP-VK.texture.compressed.bc*` now genuinely `Pass`es -- but the feature bit itself stays `no`: the mandatory format table it is checked against requires `BLIT_SRC_BIT` on *all* 16 BC formats, and BC4/BC5/BC6H's own sampling-bridge targets (`R8_UNORM`/`R8G8_UNORM`/`R16G16B16A16_FLOAT`) do not fit the blit pipeline yet, tracked as new roadmap row H8o |
