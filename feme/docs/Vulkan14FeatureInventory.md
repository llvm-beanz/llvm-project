...
| feature | VK_VERSION_1_0 | `shaderStorageImageReadWithoutFormat` | no | roadmap H7j, tracked to H19: H19a's own storage-image read/write always assumes a known, declared format; H19f confirmed the compiler-side lowering already ignores the SPIR-V handle's compile-time `Format` operand entirely, so the real remaining gap is the per-format `VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT` advertisement (`Format.cpp` computes it nowhere today) plus this device-feature bit itself -- tracked as H19i |
| feature | VK_VERSION_1_0 | `shaderStorageImageWriteWithoutFormat` | no | roadmap H7j, tracked to H19, same rationale as `shaderStorageImageReadWithoutFormat` above, tracked as H19i |
