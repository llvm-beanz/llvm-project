...
| feature | VK_VERSION_1_0 | `shaderStorageImageExtendedFormats` | no | roadmap H7j, tracked to H19: H19a closed the real prerequisite (no `OpImageRead`/`OpImageWrite` lowering existed for a storage image at all) for the plain 2D, non-arrayed, non-multisampled, mandatory-format-floor case only; H19f added real pack/unpack round-trip support (`FeMeRuntimeCPU.c`) for `R16G16B16A16_{SFLOAT,UINT,SINT}`; H19h added `R16G16B16A16_{UNORM,SNORM}`, a further slice of the full mandatory extended-format list, but the bit itself stays `VK_FALSE` until the rest of that list (single/two-channel `R8`/`R16`/`R32` formats, the packed 32-bit formats) lands -- tracked as H19j |
| feature | VK_VERSION_1_0 | `shaderStorageImageMultisample` | no | roadmap H7j, tracked to H19: H19a's own scope is explicitly non-multisampled; H19g closed the plain (non-arrayed) 2D storage-image addressing side (`Sample` operand support end-to-end through the compiler and runtime); H19k closed the `feme-cpu-linearize` control-flow gap H19g's own CTS re-run surfaced (a loop's uniform trip-count check split by `StructurizeCFGPass` into a redundant "Flow" re-derivation block); H19l then closed a distinct `feme-cpu-simdize`-adjacent bug (`matchImageCall`'s own `AllKinds` table never listed `Store2DMS`/`Store2DMSI32`, silently making an otherwise-fully-correct widening path unreachable) -- a real CTS re-run of `dEQP-VK.image.load_store_multisample.2d.*` (bit forced on to probe) now shows **27/84 Pass, 0/84 Fail**, a full clean closure of the plain (non-arrayed) 2D case. The bit still stays `VK_FALSE`: probing the full group (`2d.*` + `2d_array.*`, 252 cases) surfaces 27 real `Fail`s in `2d_array.*` (`spirv.ImageWrite` legalization rejects an `Arrayed`+`MultiSampled` image dim, per H19g's own documented `isPlainMultisampled2DImage` scope narrowing) -- tracked as new roadmap row H19m, the last row blocking an honest flip to `VK_TRUE` |
