...
| feature | VK_VERSION_1_0 | `textureCompressionBC` | no | roadmap H8i: a complete, directly-unit-tested `BCDecode.h`/`.cpp` block decoder now exists for BC1-5 (12 of the 16 `VK_FORMAT_BC*` formats; BC6H/BC7 deferred to roadmap H8l/H8m respectively), but it has zero call sites yet -- `Format.cpp`/`mapVkFormat` has no BC entries, so this bit honestly stays `no` until a future wiring row (a BC counterpart to H8j) flips it |
