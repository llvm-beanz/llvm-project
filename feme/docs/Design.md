...
| `spirv.EXT.EmitMeshTasks` (a task entry's own dispatch call, `OpEmitMeshTasksEXT`'s group-count triple plus its `TaskPayloadWorkgroupEXT` operand) | *(roadmap H6q found this has no conversion pattern at all yet -- unlike `spirv.EXT.SetMeshOutputs`, which `SetMeshOutputsEXTConversionPattern` already converts)* | *(no pattern; fails to legalize)* |
