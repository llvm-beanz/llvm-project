...
| `spirv.Constant` of `spirv.array` or `spirv.matrix` type (e.g. a `const static` HLSL array or matrix) | one flat `llvm.mlir.constant`, whatever the array's nesting (roadmap L22 extends this to a matrix constant too, which converts to the identical flat-array-of-columns shape) | *(scalar/vector only; fails to legalize for an array or matrix)* |
