...
| `spirv.Constant` of `spirv.array` or `spirv.matrix` type (e.g. a `const static` HLSL array or matrix) | one `llvm.mlir.constant` whose `DenseElementsAttr` shape mirrors the destination's real array/vector nesting (roadmap L22 extends this to a matrix constant too, which converts to the identical `!llvm.array<N x vector<M>>` shape; roadmap L29 fixed a prior flat-1-D-shape bug -- a `tensor<16xf32>`-shaped attribute for a `!llvm.array<4 x vector<4xf32>>` destination silently mistranslated into a mismatched `[16 x float]` LLVM IR constant, since upstream MLIR's `convertDenseElementsAttr` reconstructs the nested constant from the attribute's own shape, not the destination type -- the fix builds a real multi-dimensional shape, using an `mlir::VectorType` shape rather than `mlir::RankedTensorType` whenever the nesting bottoms out in a vector leaf, since only upstream's `isa<VectorType>` branch reassembles each innermost chunk as a `ConstantDataVector` rather than a mismatched `ConstantDataArray`). A `spirv.CompositeExtract` on such a matrix constant never actually reaches feme's own `MatrixCompositeExtractPattern` below -- MLIR's dialect conversion driver tries constant-folding it first (`spirv::CompositeExtractOp::fold`, via `extractCompositeElement`), and roadmap L38 fixed an **upstream MLIR** bug there (not in feme's own code) where a matrix constant's single-index fold picked one flat scalar instead of a whole column, silently broken worse for a non-square matrix (`numRows != numColumns`) than a square one. Roadmap L116(d) separately closed a struct-containing `spirv.Constant` (struct-of-scalar/matrix, or array-of-struct, at any nesting depth) -- since a struct has no legal `DenseElementsAttr`/`ElementsAttr` encoding at all, `StructConstantPattern` instead decomposes one aggregate level at a time into per-member/per-element `spirv.Constant`s feeding a `spirv.CompositeConstruct`, recursing (via `setHasBoundedRewriteRecursion()`) for further struct nesting | *(scalar/vector only; fails to legalize for an array, matrix, or any struct-containing type)* |
| `spirv.CompositeConstruct` building a vector (e.g. `floatN(...)`, a `.xxx` swizzle), a struct (e.g. assembling a whole HLSL struct value before storing it in one shot), a matrix (roadmap H10d, e.g. `mat4(c0, c1, c2, c3)`), or an array (roadmap L22, e.g. a tessellation-control-shader patch function's own `HSInput[3]`-shaped output) | an `llvm.mlir.poison` seed plus one `llvm.insertelement` per lane, or one `llvm.insertvalue` per member/column/element (reassembling a real-vector constituent lane-by-lane into a roadmap-L13 tight-vector-substituted member's array type, since `llvm.bitcast` cannot itself produce an aggregate result; roadmap H101j wraps that substituted array in a uniquely-named `!llvm.struct<"feme.tight_vector"[.N], (array<...>)>` marker struct -- via `LLVMStructType::getNewIdentified` -- so a downstream consumer like `CanonicalizeStage.cpp` can positively distinguish this stand-in from a genuinely-declared, bit-identical multi-dimensional scalar array of the same shape, which is never marker-wrapped; roadmap L116(d) fixed a further gap in `convertArray` where a stride-padded scalar array element -- `convertArrayTypeIgnoringDecorations`'s own overshooting-stride byte-array widening, roadmap L17 -- caused a hard type-mismatch failure against a plain scalar constituent operand, by round-tripping the mismatched operand through a scratch `llvm.alloca`/`store`/`load` reinterpret cast before `insertvalue`) | *(no pattern; fails to legalize)* |
| A function-local (`Function` storage class) `spirv.Variable` initialized with an array or matrix value (e.g. a GLSL `const vec4 positions[4] = vec4[](...)`, roadmap H111(a)) | an `llvm.alloca` of the converted array/matrix type plus one `llvm.store` of the (already-converted, e.g. by the `spirv.Constant` row above) initializer value -- identical to upstream's own scalar/vector lowering below, just without its type restriction | upstream's own `VariablePattern` handles a scalar or vector initializer with this same `alloca`+`store` shape, but explicitly rejects (`return failure()`) any other pointee type, even though the rest of its logic is fully type-agnostic |
...
- **Sampling variants.** `spirv.ImageSampleImplicitLod` with no modifiers and
  ...
  **Roadmap L31 has since landed these three legalization patterns** (see
...
today with no changes needed, since MLIR's own `spirv` dialect already
models every `Atomic*` instruction generically over any `SPIRV_AnyPtr`
(`SPIRVAtomicOps.td`'s `SPIRV_AtomicUpdateOp`/`SPIRV_AtomicUpdateWithValueOp`
base classes).
...
instruction's own `$pointer` operand. This instruction had **zero**
representation anywhere in MLIR's own `spirv` dialect (roadmap R39): no op
definition and no named opcode-enum case in `SPIRVBase.td` at all -- **now
closed**, adding `spirv.ImageTexelPointer` (`SPIRVImageOps.td`) and its
`SPIRV_OC_OpImageTexelPointer` opcode entry (value 60), mirroring
`ImageReadOp`'s shape but with `$image` typed as a pointer-to-image per
this one instruction's own operand layout. As expected, MLIR's
deserializer autogen infrastructure needed no hand-written code at all for
the new op -- confirmed by a real serialize/deserialize round-trip test
(`mlir/test/Target/SPIRV/image-ops.mlir`) exercising `ImageTexelPointer`
followed by `AtomicIAdd`.

The `feme`-side conversion work R39 also scoped is likewise now done: a
`SPIRVToLLVMPatterns.cpp` `ImageTexelPointerPattern` lowers the new op into
the same `createResourcePointer` (`llvm.spv.resource.getpointer`)
intrinsic call `ImageReadPattern`/`ImageWritePattern` already emit,
reusing the existing `Sample`-operand coordinate widening
(`isMultisampled2DImage`/`appendVectorLane`, roadmap H19g/H19m) for a
multisampled storage image. Along the way, this session discovered R39's
own framing of the *next* step was wrong: MLIR's upstream `spirv` -> `llvm`
conversion has **no pattern at all** for any `Atomic*` op (confirmed by
grep over `mlir/lib/Conversion/SPIRVToLLVM/`, and by a real legalization
failure hitting this while testing) -- "the op is generic over any
pointer" describes the dialect's op *definitions*, not its *lowering*.
`feme` now supplies its own `AtomicRMWPattern`/`AtomicCompareExchangePattern`
covering every SPIR-V RMW/compare-exchange atomic kind, converting each
into an ordinary `llvm.atomicrmw`/`llvm.cmpxchg` against whatever pointer
