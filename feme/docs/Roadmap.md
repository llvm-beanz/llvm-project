...
| ~~Images, image views, layout tracking, copies, storage/sampled images and samplers~~ (closed, scoped, by V5's `lib/Vulkan/Image.{h,cpp}` plus the `Descriptor.{h,cpp}`/`CommandBuffer.{h,cpp}` extensions it needed; still blocked on the rest of G2 for actual shader-side consumption -- see FeMeVulkanDesign.md's "V5" Status note) | V5 | P1 (blocked on G2) |
