...
struct BuiltinTypeDeclBuilder {
  ...
  static CallExpr *emitResourceIDCall(ASTContext &AST, Sema &S,
                                      ResourceClass RC) {
    DeclRefExpr *Fn =
        lookupBuiltinFunction(AST, S, "__builtin_hlsl_generate_resource_id");

    return CallExpr::Create(AST, Fn, {emitResourceClassExpr(AST, RC)},
                            AST.UnsignedIntTy, VK_PRValue, SourceLocation(),
                            FPOptionsOverride());
  }
  ...
  BuiltinTypeDeclBuilder &addDefaultHandleConstructor(Sema &S,
    ...
    CallExpr *Call = CallExpr::Create(
        AST, Fn,
        {emitResourceClassExpr(AST, RC), emitResourceIDCall(AST, S, RC)},
