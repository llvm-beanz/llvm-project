...
[numthreads("1",2,3)]
// expected-error@+1 {{argument to numthreads attribute must be in range 0-1024}}
[numthreads(-1,2,3)]
// expected-error@+1 {{argument to numthreads attribute must be in range 0-1024}}
[numthreads(1,-2,3)]
// expected-error@+1 {{argument to numthreads attribute must be in range 0-1024}}
[numthreads(1,2,-3)]
// expected-error@+1 {{total number of threads must not exceed 1024}}
[numthreads(1024,1024,1024)]
#endif
// CHECK: HLSLNumThreadsAttr 0x{{[0-9a-fA-F]+}} <line:{{[0-9]+}}:2, col:18> 1 2 3
[numthreads(1,2,3)]
int entry() {
 return 1;
}
